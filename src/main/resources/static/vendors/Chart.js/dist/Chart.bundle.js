...
;(function (global, factory) {
    ...
    // Return a moment from input, that is local/utc/zone equivalent to mdl.
    function cloneWithOffset(input, mdl) {
        var res, diff;
        if (mdl._isUTC) {
            res = mdl.clone();
...
module.exports = function(Chart) {
	...
	Chart.controllers.bar = Chart.DatasetController.extend({
		...
		setHoverStyle: function(rectangle) {
			...
			var custom = rectangle.custom || {};
			var mdl = rectangle._model;
			mdl.backgroundColor = custom.hoverBackgroundColor ? custom.hoverBackgroundColor : helpers.getValueAtIndexOrDefault(dataset.hoverBackgroundColor, index, helpers.getHoverColor(mdl.backgroundColor));
			mdl.borderColor = custom.hoverBorderColor ? custom.hoverBorderColor : helpers.getValueAtIndexOrDefault(dataset.hoverBorderColor, index, helpers.getHoverColor(mdl.borderColor));
			mdl.borderWidth = custom.hoverBorderWidth ? custom.hoverBorderWidth : helpers.getValueAtIndexOrDefault(dataset.hoverBorderWidth, index, mdl.borderWidth);
		},
		...
		removeHoverStyle: function(rectangle) {
			...
			mdl.backgroundColor = custom.backgroundColor ? custom.backgroundColor : helpers.getValueAtIndexOrDefault(dataset.backgroundColor, index, rectangleElementOptions.backgroundColor);
			mdl.borderColor = custom.borderColor ? custom.borderColor : helpers.getValueAtIndexOrDefault(dataset.borderColor, index, rectangleElementOptions.borderColor);
			mdl.borderWidth = custom.borderWidth ? custom.borderWidth : helpers.getValueAtIndexOrDefault(dataset.borderWidth, index, rectangleElementOptions.borderWidth);
		}
...
module.exports = function(Chart) {
	...
	Chart.controllers.bubble = Chart.DatasetController.extend({
		...
		updateElement: function(point, index, reset) {
			...
			var mdl = point._model;
			mdl.skip = custom.skip ? custom.skip : (isNaN(mdl.x) || isNaN(mdl.y));
		...
		setHoverStyle: function(point) {
			...
			mdl.radius = custom.hoverRadius ? custom.hoverRadius : (helpers.getValueAtIndexOrDefault(dataset.hoverRadius, index, this.chart.options.elements.point.hoverRadius)) + this.getRadius(this.getDataset().data[point._index]);
			mdl.backgroundColor = custom.hoverBackgroundColor ? custom.hoverBackgroundColor : helpers.getValueAtIndexOrDefault(dataset.hoverBackgroundColor, index, helpers.getHoverColor(mdl.backgroundColor));
			mdl.borderColor = custom.hoverBorderColor ? custom.hoverBorderColor : helpers.getValueAtIndexOrDefault(dataset.hoverBorderColor, index, helpers.getHoverColor(mdl.borderColor));
			mdl.borderWidth = custom.hoverBorderWidth ? custom.hoverBorderWidth : helpers.getValueAtIndexOrDefault(dataset.hoverBorderWidth, index, mdl.borderWidth);
		},
		...
		removeHoverStyle: function(point) {
			...
			mdl.radius = custom.radius ? custom.radius : this.getRadius(dataset.data[point._index]);
			mdl.backgroundColor = custom.backgroundColor ? custom.backgroundColor : helpers.getValueAtIndexOrDefault(dataset.backgroundColor, index, pointElementOptions.backgroundColor);
			mdl.borderColor = custom.borderColor ? custom.borderColor : helpers.getValueAtIndexOrDefault(dataset.borderColor, index, pointElementOptions.borderColor);
			mdl.borderWidth = custom.borderWidth ? custom.borderWidth : helpers.getValueAtIndexOrDefault(dataset.borderWidth, index, pointElementOptions.borderWidth);
		}
	});
};
...
module.exports = function(Chart) {
	...
	Chart.controllers.doughnut = Chart.controllers.pie = Chart.DatasetController.extend({
		...
		updateElement: function(arc, index, reset) {
			...
			var mdl = arc._model;
			mdl.backgroundColor = custom.backgroundColor ? custom.backgroundColor : valueAtIndexOrDefault(dataset.backgroundColor, index, arcOpts.backgroundColor);
			mdl.hoverBackgroundColor = custom.hoverBackgroundColor ? custom.hoverBackgroundColor : valueAtIndexOrDefault(dataset.hoverBackgroundColor, index, arcOpts.hoverBackgroundColor);
			mdl.borderWidth = custom.borderWidth ? custom.borderWidth : valueAtIndexOrDefault(dataset.borderWidth, index, arcOpts.borderWidth);
			mdl.borderColor = custom.borderColor ? custom.borderColor : valueAtIndexOrDefault(dataset.borderColor, index, arcOpts.borderColor);

			// Set correct angles if not resetting
			if (!reset || !animationOpts.animateRotate) {
				if (index === 0) {
					mdl.startAngle = opts.rotation;
				} else {
					mdl.startAngle = _this.getMeta().data[index - 1]._model.endAngle;
				}

				mdl.endAngle = mdl.startAngle + mdl.circumference;
			}
...
module.exports = function(Chart) {
	...
	Chart.controllers.line = Chart.DatasetController.extend({
		...
		updateBezierControlPoints: function() {
			...
			var points = meta.data || [];
			var i, ilen, point, mdl, controlPoints;
			...
			for (i=0, ilen=points.length; i<ilen; ++i) {
				...
				mdl.controlPointPreviousX = Math.max(Math.min(controlPoints.previous.x, area.right), area.left);
				mdl.controlPointPreviousY = Math.max(Math.min(controlPoints.previous.y, area.bottom), area.top);
				mdl.controlPointNextX = Math.max(Math.min(controlPoints.next.x, area.right), area.left);
				mdl.controlPointNextY = Math.max(Math.min(controlPoints.next.y, area.bottom), area.top);
		...
		setHoverStyle: function(point) {
			...
			mdl.radius = custom.hoverRadius || helpers.getValueAtIndexOrDefault(dataset.pointHoverRadius, index, this.chart.options.elements.point.hoverRadius);
			mdl.backgroundColor = custom.hoverBackgroundColor || helpers.getValueAtIndexOrDefault(dataset.pointHoverBackgroundColor, index, helpers.getHoverColor(mdl.backgroundColor));
			mdl.borderColor = custom.hoverBorderColor || helpers.getValueAtIndexOrDefault(dataset.pointHoverBorderColor, index, helpers.getHoverColor(mdl.borderColor));
			mdl.borderWidth = custom.hoverBorderWidth || helpers.getValueAtIndexOrDefault(dataset.pointHoverBorderWidth, index, mdl.borderWidth);
		},
		...
		removeHoverStyle: function(point) {
			...
			mdl.radius = custom.radius || helpers.getValueAtIndexOrDefault(dataset.pointRadius, index, me.chart.options.elements.point.radius);
			mdl.backgroundColor = me.getPointBackgroundColor(point, index);
			mdl.borderColor = me.getPointBorderColor(point, index);
			mdl.borderWidth = me.getPointBorderWidth(point, index);
		}
	});
};
	...
	Chart.controllers.radar = Chart.DatasetController.extend({
		...
		updateBezierControlPoints: function() {
			...
			helpers.each(meta.data, function(point, index) {
				...
				// Prevent the bezier going outside of the bounds of the graph
				mdl.controlPointPreviousX = Math.max(Math.min(controlPoints.previous.x, chartArea.right), chartArea.left);
				mdl.controlPointPreviousY = Math.max(Math.min(controlPoints.previous.y, chartArea.bottom), chartArea.top);

				mdl.controlPointNextX = Math.max(Math.min(controlPoints.next.x, chartArea.right), chartArea.left);
				mdl.controlPointNextY = Math.max(Math.min(controlPoints.next.y, chartArea.bottom), chartArea.top);
		...
		setHoverStyle: function(point) {
			...
			mdl.radius = custom.hoverRadius ? custom.hoverRadius : helpers.getValueAtIndexOrDefault(dataset.pointHoverRadius, index, this.chart.options.elements.point.hoverRadius);
			mdl.backgroundColor = custom.hoverBackgroundColor ? custom.hoverBackgroundColor : helpers.getValueAtIndexOrDefault(dataset.pointHoverBackgroundColor, index, helpers.getHoverColor(mdl.backgroundColor));
			mdl.borderColor = custom.hoverBorderColor ? custom.hoverBorderColor : helpers.getValueAtIndexOrDefault(dataset.pointHoverBorderColor, index, helpers.getHoverColor(mdl.borderColor));
			mdl.borderWidth = custom.hoverBorderWidth ? custom.hoverBorderWidth : helpers.getValueAtIndexOrDefault(dataset.pointHoverBorderWidth, index, mdl.borderWidth);
		},
		...
		removeHoverStyle: function(point) {
			...
			mdl.radius = custom.radius ? custom.radius : helpers.getValueAtIndexOrDefault(dataset.radius, index, pointElementOptions.radius);
			mdl.backgroundColor = custom.backgroundColor ? custom.backgroundColor : helpers.getValueAtIndexOrDefault(dataset.pointBackgroundColor, index, pointElementOptions.backgroundColor);
			mdl.borderColor = custom.borderColor ? custom.borderColor : helpers.getValueAtIndexOrDefault(dataset.pointBorderColor, index, pointElementOptions.borderColor);
			mdl.borderWidth = custom.borderWidth ? custom.borderWidth : helpers.getValueAtIndexOrDefault(dataset.pointBorderWidth, index, pointElementOptions.borderWidth);
		}
	});
};
...
module.exports = function(Chart) {
	...
	helpers.extend(Chart.DatasetController.prototype, {
		...
		removeHoverStyle: function(element, elementOpts) {
			...
			mdl.backgroundColor = custom.backgroundColor ? custom.backgroundColor : valueOrDefault(dataset.backgroundColor, index, elementOpts.backgroundColor);
			mdl.borderColor = custom.borderColor ? custom.borderColor : valueOrDefault(dataset.borderColor, index, elementOpts.borderColor);
			mdl.borderWidth = custom.borderWidth ? custom.borderWidth : valueOrDefault(dataset.borderWidth, index, elementOpts.borderWidth);
		},
		...
		setHoverStyle: function(element) {
			...
			mdl.backgroundColor = custom.hoverBackgroundColor ? custom.hoverBackgroundColor : valueOrDefault(dataset.hoverBackgroundColor, index, getHoverColor(mdl.backgroundColor));
			mdl.borderColor = custom.hoverBorderColor ? custom.hoverBorderColor : valueOrDefault(dataset.hoverBorderColor, index, getHoverColor(mdl.borderColor));
			mdl.borderWidth = custom.hoverBorderWidth ? custom.hoverBorderWidth : valueOrDefault(dataset.hoverBorderWidth, index, mdl.borderWidth);
		}
	});
...
module.exports = function(Chart) {
	...
	helpers.inherits = function(extensions) {
		//Basic javascript inheritance based on the mdl created in Backbone.js
		var parent = this;
	...
	helpers.getRelativePosition = function(evt, chart) {
		...
		// the backend mdl is in unscaled coordinates. Since we are going to deal with our mdl coordinates, we go back here
