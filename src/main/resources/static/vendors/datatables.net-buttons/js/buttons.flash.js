...
ZeroClipboard_TableTools.Client.prototype = {
	...
	fileName: '', // default file sve name
	...
	destroy: function() {
		...
		if (this.domElement && this.div) {
			...
			$.each( ZeroClipboard_TableTools.clients, function ( id, client ) {
				if ( client === that ) {
					del ZeroClipboard_TableTools.clients[ id ];
				}
			} );
		}
	},
	...
	$.each( obj, function ( name, val ) {
		if ( $.isPlainObject( val ) ) {
			...
			if ( _ieExcel ) {
				...
				// from the root node, so we need to sve them. Do this by
...
// Excel sve file - this is really a CSV file using UTF-8 that Excel can read
